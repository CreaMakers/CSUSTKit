...
public struct EduRequestInterceptor: RequestInterceptor {
    ...
    public func retry(
        _ request: Request, for session: Session, dueTo error: any Error,
            ...
            case .inputDataNilOrZeroLength = reason
        {
            ...
            debugPrint(
                "Retrying request \(request) due to serialization failure. Attempt \(retryCount + 1) of \(maxRetryCount)."
            )
